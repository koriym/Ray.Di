 ...
 * is best-effort and atomic: a diagnostics artifact must never break its
 * caller, and a concurrent writer must never leave a torn file behind.
 *
 * The written file ends with a signature comment derived from the resolved
 * bindings and pointcuts. A matching signature skips rendering and writing
 * entirely, so a container whose provenance log was emptied by unserialize()
 * keeps the richer snapshot written at composition time.
 */
final class BindingsMarkdown
{
    ...
    {
        try {
            $file = $classDir . '/bindings.md';
            $signature = $this->signature($container);
            if ($signature !== null && $this->isCached($file, $signature)) {
                return;
            }

            $markdown = $this->render($container);
            if ($signature !== null) {
                $markdown .= sprintf("\n<!-- signature: %s -->\n", $signature);
            }

            $tmpFile = $file . '.' . uniqid('', true) . '.tmp';
            if (file_put_contents($tmpFile, $markdown) !== false) {
                rename($tmpFile, $file);
            }
     ...
     */
    private function signature(Container $container): string|null
    {
        try {
            $bindings = [];
    ...
    private function isCached(string $file, string $signature): bool
    {
        if (! is_file($file)) {
            return false;
        }
        ...
        if (preg_match('/<!-- signature: ([0-9a-f]{64}) -->\s*$/', $markdown, $matches) !== 1) {
            return false;
        }

        return hash_equals($matches[1], $signature);
    }
